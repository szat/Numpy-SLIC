...
class TestSLIC(unittest.TestCase):
    ...
    def test_slices_mask_fct(self):
        ...
        mat2, _ = get_slices_mask(slices_d, self.cluster_pos0, self.S, self.image_height, self.image_width, self.border)
    ...
    def test_dis_fct(self):
        ...
        slices = get_slices(self.cluster_pos0, self.S, self.data)
        slices_d =  get_slices_d(self.cluster_pos0, slices, self.S, self.M, self.data)
        _, mat2 = get_slices_mask(slices_d, self.cluster_pos0, self.S, self.image_height, self.image_width, self.border)
    ...
    def test_final_labels_fct(self):
        ...
        slices = get_slices(self.cluster_pos0, self.S, self.data)
        slices_d =  get_slices_d(self.cluster_pos0, slices, self.S, self.M, self.data)
        slices_mask, final_dis = get_slices_mask(slices_d, self.cluster_pos0, self.S, self.image_height,
                                                 self.image_width, self.border)
        mat2 = get_final_labels(slices_mask, self.cluster_pos0, self.S, self.D)
        np.testing.assert_allclose(mat, mat2, rtol=1e-6, atol=1e-6)
    ...
    def test_new_clusters_fct(self):
        ...
        slices_mask, final_dis = get_slices_mask(slices_d, self.cluster_pos0, self.S, self.image_height,
                                                 self.image_width, self.border)
        labels = get_final_labels(slices_mask, self.cluster_pos0, self.S, self.D)
